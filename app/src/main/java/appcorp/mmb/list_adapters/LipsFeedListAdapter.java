...
public class LipsFeedListAdapter extends RecyclerView.Adapter<LipsFeedListAdapter.TapeViewHolder> {
    ...
    public LipsFeedListAdapter(List<LipsDTO> data, Context context) {
        this.data = data;
    ...
    public void onBindViewHolder(final TapeViewHolder holder, int position) {
        final LipsDTO item = data.get(position);

        if (position == data.size() - 1) {
            if (data.size()-1 % 100 != 8)
                LipsFeed.addFeed(data.size() / 100 + 1);
        }
